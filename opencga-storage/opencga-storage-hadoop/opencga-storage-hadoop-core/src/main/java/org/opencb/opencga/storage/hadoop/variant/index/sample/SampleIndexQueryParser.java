...
import static org.opencb.opencga.storage.hadoop.variant.index.annotation.AnnotationIndexConverter.POP_FREQ_005;
...
public class SampleIndexQueryParser {
    ...
    protected static byte parseAnnotationMask(Query query) {
        ...
        if (query.getAsStringList(VariantQueryParam.TYPE.key()).contains(VariantType.INDEL.toString())) {
            b |= INDEL_MASK;
        }

        for (String ct : VariantQueryUtils.splitValue(query.getString(VariantDBAdaptor.QueryParams.CONSEQUENCE_TYPE.key())).getValue()) {
        ...
        // TODO: If ANNOT_PROTEIN_SUBSTITUTION, add PROTEIN_CODING_MASK
